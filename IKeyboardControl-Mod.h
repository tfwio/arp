...
public:
  IKeyboardControl(IPlugBase* pPlug, int x, int y, int minNote, int nOctaves, IBitmap* pRegularKeys, IBitmap* pSharpKey, const int *pKeyCoords = 0) :
    ...
    mOctaveWidth(pRegularKeys->W * 7), mMaxKey(nOctaves * 12), mCurrentKey(-1)
  {
  ...
  // this is the last depressed key.
  inline int GetKey() const { return mCurrentKey; }
  ...
  void SENDIMIDIMSG(bool nOn=true, int forceKey=-1)
  {
    if (forceKey > -1)
    {
      IMidiMsg *nMsg = new IMidiMsg(0);
      if (nOn) nMsg->MakeNoteOnMsg(forceKey + mMinNote, GetVelocity(), 0);
      else nMsg->MakeNoteOffMsg(forceKey + mMinNote, 0, 0);
      ((PLUG_CLASS_NAME*)mPlug)->ProcessMidiMsg(nMsg);
    }
    if (mCurrentKey < 0) return; // we should probably check to see that it doesn't exceed the last note either, no?
    //int n_offset = mPlug->GetSamplePos();
    IMidiMsg *nMsg = new IMidiMsg(0);
    if (nOn) nMsg->MakeNoteOnMsg(mCurrentKey + mMinNote, GetVelocity(), 0);
    else nMsg->MakeNoteOffMsg(mCurrentKey + mMinNote, 0, 0);
    ((PLUG_CLASS_NAME*)mPlug)->ProcessMidiMsg(nMsg);
  }

  // compensates for MinNote
  virtual bool HasKey(int k)
  {
    return ((PLUG_CLASS_NAME*)mPlug)->GetKeyStatus(k + mMinNote);
  }

  // (1) If the key is pressed, Turn it off and return.
  // (2) Send a note on for the depressed key.
  virtual void OnMouseDown(int x, int y, IMouseMod* pMod)
  {
    int key = GetMouseKey(x, y);
    if (key < 0) return;

    if (pMod->R && HasKey(key)) {
      ((PLUG_CLASS_NAME*)mPlug)->RemoveKey(key + mMinNote);
      //SENDIMIDIMSG(false, key);
      // mCurrentKey = -1
      SetDirty();
      return;
    }

    // Skip if this key is already being played (prior) using the mouse.
    if ((key == mCurrentKey) || HasKey(key)) return;
  ...
  // (*) turns off the last depressed key.
  virtual void OnMouseUp(int x, int y, IMouseMod* pMod)
  {
    if (mCurrentKey < 0) return;
  ...
  virtual void OnMouseDrag(int x, int y, int dX, int dY, IMouseMod* pMod)
  {
    ...
    if (HasKey(key) && pMod->R) {
      OnMouseDown(x, y, pMod);
      return;
    }

    // check if the key is allready pressed (OnMouseDown does this and returns if the key is pressed)
    // Skip if this key is already being played using the mouse.
    if (HasKey(key)) return;      // no note-on if the key is allready pressed

    // turning off mCurrentKey when skipping off to press another key
    if (!(mCurrentKey < 0)) SENDIMIDIMSG(false);

    OnMouseDown(x, y, pMod);
  }

  // we could likeley shift the octave or something here, 
  // or perhaps change velocity of the particular key (for arpeggiator)
  ...
  virtual bool Draw(IGraphics* pGraphics)
  {
    ...
    if (((PLUG_CLASS_NAME*)mPlug)->GetNumKeys() == 0 && mCurrentKey == -1) return true;
    ...
    while (key < mMaxKey)
    {
      // Draw the key.
      int note = key % 12;
      if (((PLUG_CLASS_NAME*)mPlug)->GetKeyStatus(key + mMinNote) || key == mCurrentKey) DrawKey(pGraphics, &r, key, note, false);
    ...
    // Draw the high C.
    if (((PLUG_CLASS_NAME*)mPlug)->GetKeyStatus(key + mMinNote) || key == mCurrentKey) DrawKey(pGraphics, &r, key, 0, false);
    ...
    while (true)
    {
      ...
      r.R = r.L + mSharpKey.W;
      if (((PLUG_CLASS_NAME*)mPlug)->GetKeyStatus(key + mMinNote) || key == mCurrentKey) DrawKey(pGraphics, &r, key, note, true);
...
protected:
  ...
  */
  ...
  int mCurrentKey, mMinNote, mMaxKey;
  double mVelocity;
};
